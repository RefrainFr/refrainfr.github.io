...
$$\mathrm{E}[(S^n_P-0)^2] \to 0$$
as $\|P\|\to 0$.

Since $\mathrm{E}[(S^n_P)^2] = \mathrm{Var}(S^n_P) + \mathrm{E}[S^n_P]^2$, it is sufficient to prove $\mathrm{Var}(S^n_P)\to 0$ and $\mathrm{E}[S^n_P]^2\to 0$ as $\|P\|\to 0$.

For a random variable $X\sim N(0,\sigma^2)$, one can show that
$$
\mathrm{E}[X^n] = \begin{cases}
    0&, \ \text{$n$ odd}\\
...
$$\mathrm{Var}(S^n_P) = \sum_{k=0}^{m-1}\mathrm{Var}((W(t_{k+1})-W(t_k))^n)$$
and by the linearity of expectations
$$\mathrm{E}[S^n_P] = \sum_{k=0}^{m-1}\mathrm{E}[(W(t_{k+1})-W(t_k))^n].$$
For any $k\in\{0,1,2,\ldots,m-1\}$ we have $W(t_{k+1})-W(t_k)\sim N(0,t_{k+1}-t_k)$. Therefore,
$$
\mathrm{E}[(W(t_{k+1})-W(t_k))^n] = \begin{cases}
    0&, \ \text{$n$ odd}\\
    (n-1)!!\cdot(t_{k+1}-t_k)^{\frac{n}{2}}&, \ \text{$n$ even}\end{cases}
$$
and
$$
\mathrm{E}[(W(t_{k+1})-W(t_k))^{2n}] =
...
$$
\begin{aligned}
    \mathrm{Var}((W(t_{k+1})-W(t_k))^n) &= \mathrm{E}[(W(t_{k+1})-W(t_k))^{2n}] - \mathrm{E}[(W(t_{k+1})-W(t_k))^n]^2\\
    &=[(2n-1)!!-((n-1)!!)^2]\cdot(t_{k+1}-t_k)^n.
\end{aligned}
$$

As we don't need the exact value of $(2n-1)!!$ and $(n-1)!!$ in this computation, we write $A = (n-1)!!$ and $B = (2n-1)!!-((n-1)!!)^2$. Since, $\|P\|=\max_{1\le k\le m-1} (t_{k+1}-t_k)$ and $A$, $B > 0$ (by the fact $n \ge 3$), we have
$$
0\le\mathrm{E}[S^n_P] = A\sum_{k=0}^{m-1}(t_{k+1}-t_k)^{\frac{n}{2}} \le A\cdot \|P\|^{\frac{n-2}{2}}\sum_{k=0}^{m-1}(t_{k+1}-t_k)=A\cdot\|P\|^{\frac{n-2}{2}}\cdot T
$$
and
$$
\begin{aligned}
    0\le\mathrm{Var}(S^n_P) = B\sum_{k=0}^{m-1}(t_{k+1}-t_k)^n \le B\|P\|^{n-1}\sum_{k=0}^{m-1}(t_{k+1}-t_k)=B\cdot\|P\|^{n-1}\cdot T.
\end{aligned}
$$

Taking $\|P\|\to 0$ implies $\mathrm{E}[S^n_P] = \mathrm{Var}(S_P^n) = 0$ by the Squeeze Theorem. Hence
$$
\lim_{\|P\|\to 0}\mathrm{E}[(S^n_P)^2] = 0 + 0 = 0.
$$
