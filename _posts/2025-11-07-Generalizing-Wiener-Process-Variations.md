...
Most probability theory students have known that when we work on partitions on the interval $[0,T]$, the Wiener process has unbounded absolute variation and finite quadratic variation given by $T$. Now, we shall make a simple generalization which gave an unsurprising result of $0$ for the $n$-th variation when $n \ge 3$!

This exercise come from my final exam on my Bachelor's course of Stochastic Calculus. Simply speaking, this was an elective course and one of the hardest class available in my school, that is Gadjah Mada University. The simple reason that I wrote this article right now since I'm currently taking another similar course in the University of Padova, that is Stochastic Analysis. As expected, the class is hard, lol.

Last week, we discussed the two first variations in class and I suddenly remembered I have done the same thing before. So yeah, as I haven't tried to update anything in my page lately, let's just try to solve this simple problem. Thank you [the internet](https://math.stackexchange.com/questions/92648/calculation-of-the-n-th-central-moment-of-the-normal-distribution-mathcaln) for providing me a such great identity that I can abuse in the future!
...
**Solusi.** We shall assume that the partition $P$ is taken over an interval $[0,T]$. For all such partition $P$, given as
...
Since $\E[(S^n_P)^2] = \var(S^n_P) + \E[S^n_P]^2$, it is sufficient to prove $\var(S^n_P)\to 0$ and $\E[S^n_P]^2\to 0$ as $\|P\|\to 0$.
...
$$
\E[(W(t_{k+1})-W(t_k))^n] = \begin{cases}
    0&, \ \text{$n$ odd}\\
...
$$
0\le\E[S^n_P] = A\sum_{k=0}^{m-1}(t_{k+1}-t_k)^{\frac{n}{2}} \le A\cdot \|P\|^{\frac{n-2}{2}}\sum_{k=0}^{m-1}(t_{k+1}-t_k)=A\cdot\|P\|^{\frac{n-2}{2}}\cdot T
$$
    and
$$
\begin{align*}
    0\le\var(S^n_P) = B\sum_{k=0}^{m-1}(t_{k+1}-t_k)^n \le B\|P\|^{n-1}\sum_{k=0}^{m-1}(t_{k+1}-t_k)=B\cdot\|P\|^{n-1}\cdot T.
\end{align*}
$$
    Taking $\|P\|\to 0$ implies $\E[S^n_P] = \var(S_P^n) = 0$ by the Squeeze Theorem. Hence
$$
\lim_{\|P\|\to 0}\E[(S^n_P)^2] = 0 + 0 = 0.
$$
